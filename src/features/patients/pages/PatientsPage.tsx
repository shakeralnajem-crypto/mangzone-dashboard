    ...
    try {
      if (patient) {
        ...
        await create.mutateAsync(
          payload as Omit<PatientInsert, 'clinic_id' | 'created_by'>
        );
      }
      onClose();
    } catch (err) {
        ...
        >
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={
                isAr
                  ? 'تنبيهات طبية، حساسية، أو ملاحظات أخرى...'
                  : 'Medical alerts, allergies, or other notes...'
              }
            />
          </div>
...
export function PatientsPage() {
  ...
  const handleDelete = async (p: Patient) => {
    if (
      !confirm(
        `${isAr ? 'حذف المريض' : 'Delete patient'} "${p.first_name} ${p.last_name}"?`
      )
    )
      return;
    ...
    >
      ...
      <div className="ds-card" style={{ padding: '18px 20px' }}>
        ...
        <div style={{ marginTop: 14, position: 'relative' }}>
          ...
          <input
            placeholder={
              isAr ? 'بحث بالاسم أو الهاتف...' : 'Search by name or phone...'
            }
            value={search}
            ...
            <tbody>
              {paginatedPatients.map((p) => (
                <tr key={p.id} className="ds-tbody-row">
                  ...
                  >
                    {p.dob
                      ? new Date(p.dob).toLocaleDateString('en-EG', {
                          dateStyle: 'medium',
                        })
                      : '—'}
                  </td>
