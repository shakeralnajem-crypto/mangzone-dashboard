...
export function PatientDetailModal({ patient, onClose }: Props) {
  ...
  const { data: appointments = [], isLoading: apptLoading } =
    usePatientAppointments(patient?.id ?? null);
  const { data: invoices = [], isLoading: invLoading } = usePatientInvoices(
    patient?.id ?? null
  );
  const { data: payments = [], isLoading: payLoading } = usePatientPayments(
    patient?.id ?? null
  );
  const { data: summary, isLoading: summaryLoading } = usePatientSummary(
    patient?.id ?? null
  );

  if (!patient) return null;

  console.debug('[PatientDetailModal] context', {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 flex flex-col max-h-[90vh]">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-slate-500 dark:text-slate-400">
                ...
                {patient.dob && (
                  <span>
                    DOB:{' '}
                    {new Date(patient.dob).toLocaleDateString('en-EG', {
                      dateStyle: 'medium',
                    })}
                  </span>
                )}
        ...
        {/* Canonical summary */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
              Visits
            </p>
            <p className="text-base font-bold text-slate-900 dark:text-slate-100">
              {summaryLoading ? '—' : (summary?.visitCount ?? 0)}
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
            ...
            <p className="text-base font-bold text-slate-900 dark:text-slate-100">
              {summaryLoading ? '—' : (summary?.completedVisitsCount ?? 0)}
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
            ...
            <p className="text-base font-bold text-green-600 dark:text-green-400">
              {summaryLoading ? '—' : formatEGP(summary?.totalPaid ?? 0)}
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
            ...
            <p className="text-base font-bold text-amber-600 dark:text-amber-400">
              {summaryLoading ? '—' : formatEGP(summary?.balanceDue ?? 0)}
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
            ...
            <p className="text-base font-bold text-slate-900 dark:text-slate-100">
              {summaryLoading
                ? '—'
                : (summary?.paymentStatus ?? 'UNPAID').replace('_', ' ')}
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
            ...
            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
              {summaryLoading
                ? '—'
                : summary?.latestAppointment?.start_time
                  ? new Date(
                      summary.latestAppointment.start_time
                    ).toLocaleDateString('en-EG', { dateStyle: 'medium' })
                  : '—'}
            </p>
          </div>
        </div>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {appointments.map((a) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_COLORS[a.status ?? 'SCHEDULED'] ?? ''}`}
                        >
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {invoices.map((inv) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_COLORS[inv.status ?? 'UNPAID'] ?? ''}`}
                        >
                  ...
                  {payments.map((p) => (
                    ...
                    >
                      <td className="px-6 py-3 text-slate-700 dark:text-slate-200">
                        {new Date(p.payment_date).toLocaleDateString('en-EG', {
                          dateStyle: 'medium',
                        })}
                      </td>
